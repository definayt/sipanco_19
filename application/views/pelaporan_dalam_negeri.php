        ...
        <div class="content-wrapper">
          ...
          <div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Pelaporan Bepergian Antar Kota</h4>
                  <p class="card-description">
                    Data Pelaporan Antar Kota Warga RT 05/RW VII Pasca Bepergian Selama Pandemi Covid-19
                  </p>
                  ...
                  <table class="table table-striped" id="tabel-data">
                    ...
                    <tbody>
                      <?php 
                        $no = 1;
                        foreach ($pelaporan_antar_kota as $pelaporan){?>
                          <tr>
                            <td><?= $no?></td>
                            ...
                            <?php if($pelaporan->country_name == "Indonesia"){?>
                              <td><?= $pelaporan->provinsi.', '.$pelaporan->kota_kabupaten?></td>
                            <?php }else{?>
                              <td><?= $pelaporan->country_name?></td>
                            <?php }?>
                            
                           <?php 
                              $tanggal_mulai = tanggal(intval($pelaporan->tanggal_mulai[8].$pelaporan->tanggal_mulai[9]), intval($pelaporan->tanggal_mulai[5].$pelaporan->tanggal_mulai[6]), intval($pelaporan->tanggal_mulai[0].$pelaporan->tanggal_mulai[1].$pelaporan->tanggal_mulai[2].$pelaporan->tanggal_mulai[3])); 

                              $tanggal_selesai = tanggal(intval($pelaporan->tanggal_selesai[8].$pelaporan->tanggal_selesai[9]), intval($pelaporan->tanggal_selesai[5].$pelaporan->tanggal_selesai[6]), intval($pelaporan->tanggal_selesai[0].$pelaporan->tanggal_selesai[1].$pelaporan->tanggal_selesai[2].$pelaporan->tanggal_selesai[3])); 
                            ...
                            <?php if($pelaporan->pemantauan <= 14){
                              ...
                              $sisa = $pelaporan->sisa;
                              ?>
                              <td><label class="badge badge-danger"><?= $pemantauan?></td>
                            <?php }else{
                              $pemantauan = 'Selesai Pemantauan';
                              $sisa = "0";
                              ?>
                              <td><label class="badge badge-success"><?= $pemantauan?></td>
                            <?php }?>
                            ...
                            <td>
                              ...
                              id="<?php echo $pelaporan->id_pelaporan.'|'.$pelaporan->nama.'|'.$pelaporan->status.'|'.$pelaporan->country_name.'|'.$pelaporan->provinsi.'|'.$pelaporan->kota_kabupaten.'|'.$tanggal_mulai.'|'.$tanggal_selesai.'|'.$pemantauan.'|'.$sisa ?>"
                              >
                                <i class="mdi mdi-eye"></i>
                              </button>

                              <button type="button" class="btn btn-gradient-danger btn-rounded btn-icon" data-toggle="modal" data-target="#modal_hapus" onclick="deletePelaporan('<?= base_url('Pelaporan/hapus/'.$pelaporan->id_pelaporan.'_'.$pelaporan->country_name)?>')">
  ...
  <script type="text/javascript">
     $(document).ready(function () {
      ...
      $('#tabel-data').dataTable({
        dom: 'Bfrtip',
        buttons: [
            { extend: 'pdf', className: 'btn btn-gradient-info', title:'Pelaporan Bepergian Antar Kota Warga RT 05 RW VII Kelurahan Genuk', messageTop: 'Tanggal : '+today },
            { extend: 'print', className: 'btn btn-gradient-success', title:'Pelaporan Bepergian Antar Kota Warga RT 05 RW VII Kelurahan Genuk' }
        ],
        "paging": true,
        "pageLength" : 10,
    ...
    $(document).ready(function() {
        ...
        $("#IsiModal").html('<table class="table"><tr></tr><tr><td width="350">Nomor Pelaporan</td><td width="10">:</td><td>'+datanya[0]+'</td></tr><tr><td>Nama Lengkap</td><td>:</td><td>'+datanya[1]+'</td></tr><tr><td>Jenis Warga</td><td>:</td><td>'+status+'</td></tr><tr><td>Riwayat Bepergian</td><td>:</td><td>'+datanya[4]+', '+datanya[5]+'</td></tr><tr><td>Tanggal Kedatangan ke Wilayah RT 05</td><td>:</td><td>'+datanya[6]+'</td></tr><tr><td>Tanggal Selesai Pemantauan</td><td>:</td><td>'+datanya[7]+'</td></tr><tr><td>Status</td><td>:</td><td>'+datanya[8]+'</td></tr><tr><td>Sisa Hari Pemantauan</td><td>:</td><td>'+datanya[9]+' Hari</td></tr></table>');
        });
